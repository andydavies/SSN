...
%Welke?
\textit{\Large "SSL session key extraction from the process memory on Android mobile devices"}
...
Public-key cryptography is used by TLS/SSL to establish a secure session. When this secure session has been established it generates short term symmetric keys for each data connection in this session.

This paper will cover how the tool Frida was used to extract these symmetric keys from a running Android application and how these keys where subsequently used to decrypt captured traffic.}
